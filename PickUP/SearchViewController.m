...
//    [[UIBarButtonItem appearanceWhenContainedIn: [UISearchBar class], nil] setTintColor:[UIColor whiteColor]];
    ...
    self.selectedLocation = [SelectedLocation new];
...
-(void)retrieveGooglePlaceInformation:(NSString *)searchWord withCompletion:(void (^)(NSArray *))complete{
    ...
    if (searchWordProtection.length != 0) {
        ...
        NSString *urlString = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/autocomplete/json?input=%@&types=establishment|geocode&location=%@,%@&radius=500&language=en&key=%@",searchWord,currentLatitude,currentLongitude,API_KEY];
...
-(void)retrieveJSONDetailsAbout:(NSString *)place withCompletion:(void (^)(NSArray *))complete {
    NSString *urlString = [NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/place/details/json?placeid=%@&key=%@", place, API_KEY];
    NSURL *url = [NSURL URLWithString:[urlString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
    NSURLSessionConfiguration *defaultConfigObject = [NSURLSessionConfiguration defaultSessionConfiguration];
    NSURLSession *delegateFreeSession = [NSURLSession sessionWithConfiguration: defaultConfigObject delegate: nil delegateQueue: [NSOperationQueue mainQueue]];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
...
case TableViewSectionMain: {
            ...
            [self retrieveJSONDetailsAbout:placeID withCompletion:^(NSArray *place) {
                ...
                self.selectedLocation.cityName = [place valueForKey:@"vicinity"];
                ...
                [self.delegate searchedLocation:self.selectedLocation.address coordinate:self.selectedLocation.locationCoordinates cityName:self.selectedLocation.cityName];
                [self.dropOffDelegate dropOffLocation:self.selectedLocation.address coordinate:self.selectedLocation.locationCoordinates cityName:self.selectedLocation.cityName];
